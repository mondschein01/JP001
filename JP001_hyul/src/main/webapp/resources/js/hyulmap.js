...
let ary_menu_el_a = [];
let ary_submenu_el_a = [];

let el_div_root; // 루트 컨테이너
let el_div_menu; // 메뉴 컨테이너
let el_div_submenu; // 서브메뉴 컨테이너
let el_div_box_desc; // description 박스
...
function mInit() {
    // 엘리먼트 생성에 필요한 모체 먼저 찾아서 넘겨주기
    el_div_root = document.querySelector("#root");
    el_div_menu = document.querySelector("#menu");
    el_div_submenu = document.querySelector("#submenu");
    el_div_box_desc = document.querySelector("#box_desc");

    el_category = el_div_box_desc.children.item(0);
    el_hyulName = el_div_box_desc.children.item(1);
    el_hyulPos = el_div_box_desc.children.item(3);
    el_hyulSelect = el_div_box_desc.children.item(5);
    el_wayToChim = el_div_box_desc.children.item(7);
    el_hyulEffect = el_div_box_desc.children.item(9);

    el_div_box_desc.children.item(2).innerHTML = "부위";
    el_div_box_desc.children.item(4).innerHTML = "취혈";
    el_div_box_desc.children.item(6).innerHTML = "침구법";
    el_div_box_desc.children.item(8).innerHTML = "주치증";

    // 메뉴 만들기
    makeMenu();

    // 서브 메뉴 만들기
    makeSubMenu(ary_menu_el_a[0], 0);
}

// 혈 이름 배열을 이용해서 div#root > div#menu > ul에 넣기
function makeMenu() {
    for (let i = 0; i < sAry_hyuls.length; i++) {
        ...
        // 메뉴 a 배열에 a 태그 넣고
        ary_menu_el_a.push(tmp_el_a);

        // a 태그에 혈자리 이름 넣고
        tmp_el_a.innerText = sAry_hyuls[i];

        // 메뉴 안의 a 태그에 클릭하면 서브메뉴 만드는 메소드 등록하고
        tmp_el_a.addEventListener('click', () => makeSubMenu(tmp_el_a, i));

        // li 태그에 a 태그 넣고
        tmp_el_li.appendChild(tmp_el_a)

        // 메뉴 div안의 ul에 li를 넣는다
        el_div_menu.firstElementChild.appendChild(tmp_el_li);
    }
}
...
function makeSubMenu(item, idx) {
    ...
    // 기존 서브 메뉴가 있다면 삭제
    if (el_div_submenu.firstChild != null) {
        el_div_submenu.firstChild.remove();
    }
    ...
    for (let i = 0; i < tgAry.length; i++) {
        ...
        // submenu_a 배열에 a 넣기
        ary_submenu_el_a.push(tmp_el_a);
    ...
    // 서브메뉴에 ul 추가
    el_div_submenu.appendChild(tmp_el_ul);

    // 다른 item 활성화 삭제
    for (let i = 0; i < ary_menu_el_a.length; i++) {
        ary_menu_el_a[i].classList.remove("activate");
    }
    ...
    // 서브 메뉴 활성화
    activateSubMenu(ary_submenu_el_a[0], sAry_hyuls[idx], tgAry[0]);
}

// 서브 메뉴의 클래스에 activate 넣어서 css로 활성 효과 받게 하기
function activateSubMenu(item, categoryName, hyulName) {

    // 다른 item 활성화 삭제
    for (let i = 0; i < ary_submenu_el_a.length; i++) {
        ary_submenu_el_a[i].classList.remove("activate");
...
// Servlet으로 AJAX 요청 // json으로 보낼까...
function dataTrans(categoryName, hyulName){
	let xhr = new XMLHttpRequest();
	// xhr.open("POST", "/JP001_hyul/map", true); // post
    // let data = `cName=${categoryName}&hName=${hyulName}`;
	xhr.open("GET", `/JP001_hyul/map?cName=${categoryName}&hName=${hyulName}`, true); // get으로 보내자
...
// description 박스 안의 이미지와 텍스트 업데이트
